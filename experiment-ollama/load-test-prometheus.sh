...
print_current_power_usage() {
  local response
  local sample_count

  if ! response="$(curl -fsS --get \
    --data-urlencode "query=$POWER_METRIC" \
    "$PROMETHEUS_URL/api/v1/query")"; then
    log "ERROR: cannot query $POWER_METRIC from $PROMETHEUS_URL"
    return 1
  fi

  if [[ "$(jq -r '.status // empty' <<< "$response")" != "success" ]]; then
    log "ERROR: Prometheus returned an unsuccessful response for $POWER_METRIC"
    jq -c . <<< "$response"
    return 1
  fi

  sample_count="$(jq '.data.result | length' <<< "$response")"
  if [[ "$sample_count" -eq 0 ]]; then
    log "ERROR: Prometheus returned no current samples for $POWER_METRIC"
    return 1
  fi

  log "Current $POWER_METRIC values:"
  jq -r --arg metric "$POWER_METRIC" \
    '.data.result[] | "\($metric) \(.metric | del(.__name__) | tojson) \(.value[1]) W"' \
    <<< "$response"
}

print_current_power_usage || exit 1
